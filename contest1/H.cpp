...
void solve() {
    ...
    std::vector<std::vector<long long>> dp(n, std::vector<long long>(1LL << n, -INF));
    initialize_dp(n, dp);
    for (long long i = 0; i < n - 1; i++) {
        for (long long mask = 0; mask < (1LL << (i + 1)); mask++) {
    ...
    std::cout << compute_maxi(n, dp) << std::endl;
}
